...
| **Date** | 2022-12-20T21:45:21+09:00 |
| **Message-ID** | [20221220124521.499-1-takashi.yano@nifty.ne.jp](https://inbox.sourceware.org/cygwin-patches/20221220124521.499-1-takashi.yano@nifty.ne.jp) |

---

**Thread**: [thread](12-45-21.thread.md#:~:text=2022/12/20/12-45-21)
...
index a0762f292..d3c19716a 100644
--- a/winsup/cygwin/devices.cc
+++ b/winsup/cygwin/devices.cc
@@ -67,6 +67,21 @@ exists_ntdev_silent (const device& dev)
...
index 7506dfe9c..932bb882c 100644
--- a/winsup/cygwin/devices.in
+++ b/winsup/cygwin/devices.in
@@ -63,6 +63,21 @@ exists_ntdev_silent (const device& dev)
   return exists_ntdev (dev) ? -1 : false;
 }
...
+
 ...
 exists_console (const device& dev)
 {
@@ -74,8 +89,13 @@ exists_console (const device& dev)
     case FH_CONOUT:
       return fhandler_console::exists ();
     default:
-      /* Only show my own console device (for now?) */
...
index bbf4b0103..c2b0666df 100644
...
@@ -65,6 +66,48 @@ static struct fhandler_base::rabuf_t con_ra;
    in xterm compatible mode */
 static wchar_t last_char;
 
+DWORD
+fhandler_console::attach_console (pid_t owner)
+{
+  DWORD resume_pid = MAX_PID + 1;
+  pinfo p (owner);
+  if (p)
+    {
+      DWORD attached =
...
+
 ...
 beep ()
 {
@@ -155,7 +210,7 @@ fhandler_console::open_shared_console (HWND hw, HANDLE& h, bool& create)
...
@@ -182,7 +237,7 @@ enum_windows (HWND hw, LPARAM lp)
   fhandler_console::console_state *cs;
   if ((cs = fhandler_console::open_shared_console (hw, h)))
     {
...
@@ -190,12 +245,12 @@ enum_windows (HWND hw, LPARAM lp)
 }
...
+    api_fatal ("console device allocation failure - too many consoles in use, max consoles is 64");
 ...
 static DWORD
@@ -286,6 +341,7 @@ inrec_eq (const INPUT_RECORD *a, const INPUT_RECORD *b, DWORD n)
 void
...
@@ -436,7 +492,7 @@ fhandler_console::cons_master_thread (handle_set_t *p, tty *ttyp)
...
+	}
   ...
   dev ().parse (devset);
@@ -651,7 +753,7 @@ fhandler_console::setup ()
...
@@ -736,6 +840,7 @@ fhandler_console::set_input_mode (tty::cons_mode m, const termios *t,
...
@@ -763,7 +869,9 @@ fhandler_console::set_output_mode (tty::cons_mode m, const termios *t,
       break;
     }
   acquire_attach_mutex (mutex_timeout);
+  DWORD resume_pid = attach_console (con.owner);
...
@@ -786,9 +894,10 @@ fhandler_console::setup_for_non_cygwin_app ()
 void
...
@@ -851,6 +961,10 @@ fhandler_console::setup_io_mutex (void)
     }
...
+    attach_mutex = CreateMutex (&sec_none_nih, FALSE, NULL);
 ...
 inline DWORD
@@ -893,7 +1007,9 @@ fhandler_console::set_cursor_maybe ()
...
@@ -902,15 +1018,17 @@ fhandler_console::set_cursor_maybe ()
...
@@ -927,7 +1045,7 @@ fhandler_console::send_winch_maybe ()
       con.scroll_region.Bottom = -1;
       if (wincap.has_con_24bit_colors () && !con_is_legacy
 	  && wincap.has_con_broken_tabs ())
...
@@ -945,7 +1063,9 @@ fhandler_console::mouse_aware (MOUSE_EVENT_RECORD& mouse_event)
...
+	      detach_console (resume_pid, con.owner);
 	      ...
 	      if (res)
 		goto wait_retry;
@@ -1094,7 +1216,7 @@ fhandler_console::process_input_message (void)
 {
   char tmp[60];
...
@@ -1104,8 +1226,10 @@ fhandler_console::process_input_message (void)
...
+  fhandler_console::detach_console (resume_pid, owner);
   ...
   if (ret)
@@ -1560,7 +1688,9 @@ dev_console::scroll_buffer (HANDLE h, int x1, int y1, int x2, int y2,
...
@@ -1612,9 +1742,11 @@ fhandler_console::open (int flags, mode_t)
...
+  detach_console (resume_pid, con.owner);
   ...
   if (h == INVALID_HANDLE_VALUE)
     {
@@ -1624,9 +1756,11 @@ fhandler_console::open (int flags, mode_t)
   set_handle (h);
...
+  detach_console (resume_pid, con.owner);
   ...
   if (h == INVALID_HANDLE_VALUE)
     {
@@ -1635,12 +1769,14 @@ fhandler_console::open (int flags, mode_t)
     }
   set_output_handle (h);
...
@@ -1657,6 +1793,7 @@ fhandler_console::open (int flags, mode_t)
       DWORD dwMode;
...
@@ -1670,6 +1807,7 @@ fhandler_console::open (int flags, mode_t)
...
+      detach_console (resume_pid, con.owner);
       ...
       extern int sawTERM;
@@ -1725,14 +1863,15 @@ fhandler_console::close ()
...
@@ -1744,7 +1883,7 @@ fhandler_console::close ()
...
+  if (shared_console_info[unit] && con.owner == myself->pid
     ...
     {
       char name[MAX_PATH];
@@ -1766,9 +1905,15 @@ fhandler_console::close ()
...
@@ -1843,7 +1988,9 @@ fhandler_console::ioctl (unsigned int cmd, void *arg)
 	  int ret = 0;
...
+	  detach_console (resume_pid, con.owner);
 	  ...
 	  if (!r)
 	    {
@@ -1902,7 +2049,9 @@ fhandler_console::tcflush (int queue)
       || queue == TCIOFLUSH)
     {
       acquire_attach_mutex (mutex_timeout);
+      DWORD resume_pid = attach_console (con.owner);
...
+      detach_console (resume_pid, con.owner);
       ...
       if (!r)
 	{
@@ -1929,15 +2078,14 @@ fhandler_console::tcgetattr (struct termios *t)
   return 0;
 }
...
-  _tc = &(shared_console_info->tty_min_state);
 ...
 void
@@ -1978,7 +2126,9 @@ dev_console::set_color (HANDLE h)
   if (h)
     {
       acquire_attach_mutex (mutex_timeout);
+      DWORD resume_pid = fhandler_console::attach_console (owner);
...
@@ -2035,6 +2185,7 @@ dev_console::scroll_window (HANDLE h, int x1, int y1, int x2, int y2)
...
@@ -2089,6 +2240,7 @@ dev_console::scroll_window (HANDLE h, int x1, int y1, int x2, int y2)
...
+  fhandler_console::detach_console (resume_pid, owner);
   ...
   fillin (h);
@@ -2147,8 +2299,10 @@ dev_console::clear_screen (HANDLE h, int x1, int y1, int x2, int y2)
       tlc.Y = y2;
     }
   acquire_attach_mutex (mutex_timeout);
+  DWORD resume_pid = fhandler_console::attach_console (owner);
...
@@ -2183,7 +2337,9 @@ fhandler_console::cursor_set (bool rel_to_top, int x, int y)
   pos.X = x;
   pos.Y = y;
   acquire_attach_mutex (mutex_timeout);
+  DWORD resume_pid = attach_console (con.owner);
...
@@ -2257,7 +2413,9 @@ fhandler_console::write_console (PWCHAR buf, DWORD len, DWORD& done)
     {
...
+      detach_console (resume_pid, con.owner);
       ...
       if (!r)
 	{
@@ -2309,9 +2467,7 @@ ReadConsoleOutputWrapper (HANDLE h, PCHAR_INFO buf, COORD bufsiz,
   if ((width == 0) || (height == 0))
     return TRUE;
...
@@ -2347,14 +2503,20 @@ dev_console::save_restore (HANDLE h, char c)
...
+      fhandler_console::detach_console (resume_pid, owner);
       ...
       /* Clear entire buffer */
@@ -2370,7 +2532,9 @@ dev_console::save_restore (HANDLE h, char c)
...
@@ -2383,11 +2547,13 @@ dev_console::save_restore (HANDLE h, char c)
...
+      fhandler_console::detach_console (resume_pid, owner);
       ...
       dwEnd.X = dwEnd.Y = 0;
@@ -2503,24 +2669,26 @@ fhandler_console::char_command (char c)
 	  break;
 #endif
 	case 'b': /* REP */
-	  wpbuf.put (c);
...
@@ -2532,11 +2700,14 @@ fhandler_console::char_command (char c)
...
+	      DWORD resume_pid = attach_console (con.owner);
 		...
 		{
@@ -2552,8 +2723,8 @@ fhandler_console::char_command (char c)
...
+	      wpbuf_send ();
 	      __small_swprintf (bufw, L"\033[%d;%dr",
 				...
 				srBottom + 1 - con.b.srWindow.Top);
@@ -2561,13 +2732,14 @@ fhandler_console::char_command (char c)
...
+	      wpbuf_send ();
 	  ...
 	  break;
 	case 'M': /* DL */
@@ -2580,7 +2752,9 @@ fhandler_console::char_command (char c)
 	      if (y == con.b.srWindow.Bottom)
 		{
 		  acquire_attach_mutex (mutex_timeout);
+		  DWORD resume_pid = attach_console (con.owner);
...
@@ -2588,9 +2762,10 @@ fhandler_console::char_command (char c)
 				y + 1 - con.b.srWindow.Top,
 				srBottom + 1 - con.b.srWindow.Top);
 	      acquire_attach_mutex (mutex_timeout);
+	      DWORD resume_pid = attach_console (con.owner);
...
+	      wpbuf_send ();
 	      __small_swprintf (bufw, L"\033[%d;%dr",
 				srTop + 1 - con.b.srWindow.Top,
 				srBottom + 1 - con.b.srWindow.Top);
@@ -2598,17 +2773,18 @@ fhandler_console::char_command (char c)
 	      __small_swprintf (bufw, L"\033[%d;%dH",
 				y + 1 - con.b.srWindow.Top, x + 1);
 	      WriteConsoleW (get_output_handle (), bufw, wcslen (bufw), 0, 0);
+	      detach_console (resume_pid, con.owner);
...
@@ -2618,6 +2794,7 @@ fhandler_console::char_command (char c)
...
@@ -2629,17 +2806,18 @@ fhandler_console::char_command (char c)
...
@@ -2655,7 +2833,7 @@ fhandler_console::char_command (char c)
 		}
...
+		fix_tab_position (get_output_handle (), con.owner);
 	  ...
 	  break;
 	case 'p':
@@ -2666,23 +2844,23 @@ fhandler_console::char_command (char c)
...
+	  wpbuf_send ();
       ...
       return;
@@ -2867,6 +3045,7 @@ fhandler_console::char_command (char c)
 	{
...
+	    DWORD resume_pid = attach_console (con.owner);
 	    ...
 	    switch (con.args[0])
 	      {
@@ -2889,6 +3068,7 @@ fhandler_console::char_command (char c)
 					&console_cursor_info);
 		  break;
 	      }
+	    detach_console (resume_pid, con.owner);
       ...
       break;
@@ -2912,12 +3092,14 @@ fhandler_console::char_command (char c)
 	  {
 	    CONSOLE_CURSOR_INFO console_cursor_info;
 	    acquire_attach_mutex (mutex_timeout);
+	    DWORD resume_pid = attach_console (con.owner);
...
@@ -3164,14 +3346,16 @@ enum_proc (const LOGFONTW *lf, const TEXTMETRICW *tm,
 }
...
+  fhandler_console::detach_console (resume_pid, owner);
   ...
   if (!r)
     return;
@@ -3242,11 +3426,13 @@ check_font (HANDLE hdl)
 inline void
...
@@ -3517,7 +3707,7 @@ fhandler_console::write (const void *vsrc, size_t len)
 	case gotesc:
...
+	      wpbuf_put (*src);
 	      ...
 	      con.nargs = 0;
@@ -3532,8 +3722,8 @@ fhandler_console::write (const void *vsrc, size_t len)
 		{
 		  /* For xterm mode only */
 		  /* Just send the sequence */
-		  wpbuf.put (*src);
...
@@ -3546,8 +3736,8 @@ fhandler_console::write (const void *vsrc, size_t len)
 		{
 		  /* For xterm mode only */
 		  /* Just send the sequence */
-		  wpbuf.put (*src);
...
@@ -3570,23 +3760,25 @@ fhandler_console::write (const void *vsrc, size_t len)
...
@@ -3603,20 +3795,20 @@ fhandler_console::write (const void *vsrc, size_t len)
...
@@ -3654,19 +3846,19 @@ fhandler_console::write (const void *vsrc, size_t len)
 	    {
...
@@ -3700,26 +3892,26 @@ fhandler_console::write (const void *vsrc, size_t len)
...
+		  wpbuf_send ();
 		...
 		con.state = normal;
@@ -3734,7 +3926,7 @@ fhandler_console::write (const void *vsrc, size_t len)
 	    break;
 	  }
 	case eatpalette:
-	  wpbuf.put (*src);
...
@@ -3744,20 +3936,20 @@ fhandler_console::write (const void *vsrc, size_t len)
...
@@ -3770,7 +3962,7 @@ fhandler_console::write (const void *vsrc, size_t len)
 	  if (*src == ';')
 	    {
 	      con.state = gotarg1;
-	      wpbuf.put (*src);
+	      wpbuf_put (*src);
 		...
 		con.nargs++;
 	      src++;
@@ -3785,7 +3977,7 @@ fhandler_console::write (const void *vsrc, size_t len)
...
@@ -3936,13 +4128,15 @@ fhandler_console::set_close_on_exec (bool val)
 }
...
@@ -4031,7 +4225,7 @@ fhandler_console::fixup_after_fork_exec (bool execing)
 {
   set_unit ();
   setup_io_mutex ();
-  wpbuf.init (get_output_handle ());
+  wpbuf.init ();
   ...
   if (!execing)
     return;
@@ -4281,6 +4475,7 @@ fhandler_console::get_duplicated_handle_set (handle_set_t *p)
...
@@ -4307,6 +4502,7 @@ fhandler_console::need_console_handler ()
 void
...
index 8c320421b..f8ae27bf5 100644
...
+  static DWORD attach_console (pid_t);
