...
| **Date** | 2020-02-18T04:03:00Z |
| **Message-ID** | [20200218040324.329-1-takashi.yano@nifty.ne.jp](https://inbox.sourceware.org/cygwin-patches/20200218040324.329-1-takashi.yano@nifty.ne.jp) |

---

**Thread**: [thread](04-03-00.thread.md#:~:text=2020/02/18/04-03-00)
...
```
 ...
 winsup/cygwin/fhandler_console.cc | 123 +++++++++++++++++++++---------
...
index 9bfee64d3..7cb45b4a7 100644
...
@@ -430,8 +484,8 @@ fhandler_console::read (void *pv, size_t& buflen)
   DWORD dwMode;
   GetConsoleMode (get_handle (), &dwMode);
...
+    request_xterm_mode_input (true);
   ...
   while (!input_ready && !get_cons_readahead_valid ())
     {
@@ -439,7 +493,8 @@ fhandler_console::read (void *pv, size_t& buflen)
...
@@ -457,7 +512,8 @@ fhandler_console::read (void *pv, size_t& buflen)
 	case WAIT_TIMEOUT:
...
+	    request_xterm_mode_input (false);
 	...
 	default:
 	  goto err;
@@ -505,19 +561,22 @@ fhandler_console::read (void *pv, size_t& buflen)
 #undef buf
...
@@ -1051,10 +1110,7 @@ fhandler_console::open (int flags, mode_t)
...
+  if (myself->pid == con.owner && wincap.has_con_24bit_colors ())
     {
       ...
       DWORD dwMode;
@@ -1105,15 +1161,9 @@ fhandler_console::close ()
...
@@ -1121,8 +1171,8 @@ fhandler_console::close ()
...
@@ -1266,6 +1316,8 @@ fhandler_console::output_tcsetattr (int, struct termios const *t)
...
@@ -1736,7 +1788,7 @@ bool fhandler_console::write_console (PWCHAR buf, DWORD len, DWORD& done)
     }
...
@@ -2704,11 +2756,12 @@ fhandler_console::write (const void *vsrc, size_t len)
   acquire_output_mutex (INFINITE);
