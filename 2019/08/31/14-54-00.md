...
| **Message-ID** | [20190831145318.1854-2-takashi.yano@nifty.ne.jp](https://inbox.sourceware.org/cygwin-patches/20190831145318.1854-2-takashi.yano@nifty.ne.jp) |

---

**Thread**: [thread](14-54-00.thread.md#:~:text=2019/08/31/14-54-00)
...
index dd5ab528a..ee2945004 100644
...
+
   ...
   if (getPseudoConsole ())
     {
@@ -1361,9 +1379,12 @@ fhandler_pty_slave::read (void *ptr, size_t& len)
 	    }
 	  goto out;
 	}
-      if (check_switch_to_pcon () &&
...
@@ -1406,8 +1427,13 @@ fhandler_pty_slave::read (void *ptr, size_t& len)
...
+do_read_cyg:
 	...
 	{
 	  ReleaseMutex (input_mutex);
@@ -1675,31 +1701,12 @@ fhandler_pty_slave::ioctl (unsigned int cmd, void *arg)
     case TIOCSWINSZ:
       if (getPseudoConsole ())
 	{
...
@@ -1717,20 +1724,9 @@ fhandler_pty_slave::ioctl (unsigned int cmd, void *arg)
...
+
 	...
 	{
@@ -2106,7 +2102,7 @@ fhandler_pty_master::close ()
...
@@ -2160,8 +2156,8 @@ fhandler_pty_master::write (const void *ptr, size_t len)
...
+      (!get_ttyp ()->mask_switch_to_pcon || ALWAYS_USE_PCON))
     {
       char *buf;
       size_t nlen;
@@ -2770,8 +2766,9 @@ fhandler_pty_slave::fixup_after_attach (bool native_maybe)
...
@@ -2856,19 +2853,20 @@ fhandler_pty_slave::fixup_after_attach (bool native_maybe)
...
+		      ENABLE_PROCESSED_INPUT);
       ...
       if (get_ttyp ()->pcon_pid == 0 ||
 	  kill (get_ttyp ()->pcon_pid, 0) != 0)
@@ -2896,23 +2894,28 @@ fhandler_pty_slave::fixup_after_exec ()
   else if (getPseudoConsole ())
     {
       int used = 0;
+      int attached = 0;
...
@@ -3119,7 +3122,7 @@ fhandler_pty_master::pty_master_fwd_thread ()
 	{
...
@@ -3153,6 +3156,24 @@ fhandler_pty_master::pty_master_fwd_thread ()
 		continue;
 	      }
