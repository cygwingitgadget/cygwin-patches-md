...
| **Message-ID** | [20190913193356.1517-2-takashi.yano@nifty.ne.jp](https://inbox.sourceware.org/cygwin-patches/20190913193356.1517-2-takashi.yano@nifty.ne.jp) |

---

**Thread**: [thread](19-34-00.thread.md#:~:text=2019/09/13/19-34-00)
...
+	  int fd = chk_order[i];
+	  fhandler_base *fh = ::cygheap->fdtab[fd];
+	  if (fh->get_major () == DEV_PTYS_MAJOR)
+	    {
+	      fhandler_pty_slave *ptys = (fhandler_pty_slave *) fh;
+	      if (n != 0
+		  || h == ptys->get_handle ()
...
+		}
+	    }
+	  else if (fh->get_major () == DEV_CONS_MAJOR)
+	    {
...
+		{
+		  /* If the process is running on console,
...
@@ -303,7 +337,7 @@ PeekConsoleInputW_Hooked
...
@@ -896,7 +930,7 @@ fhandler_pty_slave::close ()
...
@@ -985,14 +1019,16 @@ fhandler_pty_slave::restore_reattach_pcon (void)
 }
...
+  if (fd == 0 && !get_ttyp ()->switch_to_pcon_in)
     {
       pid_restore = 0;
       if (pcon_attached_to != get_minor ())
@@ -1000,15 +1036,22 @@ fhandler_pty_slave::set_switch_to_pcon (void)
...
@@ -1029,7 +1072,7 @@ fhandler_pty_slave::reset_switch_to_pcon (void)
     }
...
+    this->set_switch_to_pcon (fd);
   if (get_ttyp ()->pcon_pid &&
       get_ttyp ()->pcon_pid != myself->pid &&
       kill (get_ttyp ()->pcon_pid, 0) == 0)
@@ -1041,7 +1084,10 @@ fhandler_pty_slave::reset_switch_to_pcon (void)
 	{
 	  DWORD mode;
 	  GetConsoleMode (get_handle (), &mode);
...
@@ -1049,15 +1095,17 @@ fhandler_pty_slave::reset_switch_to_pcon (void)
     }
...
@@ -1111,7 +1159,7 @@ fhandler_pty_slave::push_to_pcon_screenbuffer (const char *ptr, size_t len)
 	    {
 	      //p0 += 8;
...
@@ -1133,7 +1181,7 @@ fhandler_pty_slave::push_to_pcon_screenbuffer (const char *ptr, size_t len)
     }
...
@@ -1209,7 +1257,7 @@ fhandler_pty_slave::write (const void *ptr, size_t len)
...
+  UINT targetCodePage = get_ttyp ()->switch_to_pcon_out ?
   ...
   if (targetCodePage != get_ttyp ()->TermCodePage)
     {
@@ -1239,7 +1287,7 @@ fhandler_pty_slave::write (const void *ptr, size_t len)
...
@@ -1248,12 +1296,12 @@ fhandler_pty_slave::write (const void *ptr, size_t len)
...
@@ -1273,7 +1321,7 @@ fhandler_pty_slave::write (const void *ptr, size_t len)
...
+  if (get_ttyp ()->switch_to_pcon_out && !fallback)
   ...
   restore_reattach_pcon ();
@@ -1292,8 +1340,8 @@ fhandler_pty_slave::write (const void *ptr, size_t len)
 bool
...
+    (!get_ttyp ()->mask_switch_to_pcon_in || ALWAYS_USE_PCON);
 ...
 void __reg3
@@ -1322,7 +1370,7 @@ fhandler_pty_slave::read (void *ptr, size_t& len)
...
@@ -1475,7 +1523,7 @@ fhandler_pty_slave::read (void *ptr, size_t& len)
 	  len = rlen;
...
+  mask_switch_to_pcon_in (false);
 ...
 int
@@ -2147,7 +2195,8 @@ fhandler_pty_master::close ()
...
@@ -2231,12 +2280,24 @@ fhandler_pty_master::write (const void *ptr, size_t len)
 	}
       DWORD wLen;
...
@@ -2887,43 +2950,58 @@ fhandler_pty_slave::fixup_after_attach (bool native_maybe)
 			  break;
 			}
 		}
+	    }
...
@@ -2932,6 +3010,12 @@ fhandler_pty_slave::fixup_after_fork (HANDLE parent)
 void
...
+
   ...
   if (!close_on_exec ())
@@ -3169,7 +3253,7 @@ fhandler_pty_master::pty_master_fwd_thread ()
 	{
