...
| **Date** | 2004-09-17T11:40:00Z |
| **Message-ID** | [Pine.CYG.4.58.0409171335200.2356@peda-pc.Axentia.local](https://inbox.sourceware.org/cygwin-patches/Pine.CYG.4.58.0409171335200.2356@peda-pc.Axentia.local) |

---

**Thread**: [thread](11-40-00.thread.md#:~:text=2004/09/17/11-40-00)
...
They are perhaps not 100% correct (I'm not aware of any errors), but they
do work for me and should at the least serve as a starting point.

I don't know what needs to be done in order to include them in the w32api
package, where I think they also belong. Perhaps some kind soul can fix
that?
...
2004-09-17  Peter Ekberg  <peda@lysator.liu.se>

	* include/directx/ddraw.h: New file.
	* include/directx/dinput.h: New file.
	* README.w32api: Add copyright notice for new files.

Index: README.w32api
===================================================================
RCS file: /cvs/src/src/winsup/w32api/README.w32api,v
retrieving revision 1.4
diff -u -u -p -r1.4 README.w32api
```diff
--- README.w32api	17 Apr 2004 13:44:05 -0000	1.4
+++ README.w32api	17 Sep 2004 10:46:58 -0000
@@ -30,3 +30,4 @@ gl.h is from the Mesa 3-D graphics libra
...
+dinput.h and ddraw.h are from the Wine project, Copyright (C) the Wine project
```

Index: include/directx/ddraw.h
===================================================================
RCS file: include/directx/ddraw.h
diff -N include/directx/ddraw.h
```diff
--- /dev/null	1 Jan 1970 00:00:00 -0000
+++ include/directx/ddraw.h	17 Sep 2004 10:46:59 -0000
@@ -0,0 +1,2141 @@
+/*
...
+
+/* Winelib compatibility section
+ * Some winelib defines are reused here, which
+ * might cause problems if trying to use winelib
+ * on cygwin.
+ * The code below that is dependent on this could
+ * be modified to not need it, but it then becomes
+ * harder to sync up with changes in the winelib
+ * version.
+ */
+#if !defined(__cplusplus) || defined(CINTERFACE)
+#define COBJMACROS
+#endif
+#undef WINELIB_NAME_AW
+#ifdef UNICODE
+# define WINELIB_NAME_AW(func) func##W
+#else
+# define WINELIB_NAME_AW(func) func##A
+#endif  /* UNICODE */
+#undef DECL_WINELIB_TYPE_AW
+#define DECL_WINELIB_TYPE_AW(type) typedef WINELIB_NAME_AW(type) type;
+#undef IUnknown_METHODS
...
+/* End of winelib compatibility section */
+
+/*****************************************************************************
...
Index: include/directx/dinput.h
===================================================================
RCS file: include/directx/dinput.h
diff -N include/directx/dinput.h
```diff
--- /dev/null	1 Jan 1970 00:00:00 -0000
+++ include/directx/dinput.h	17 Sep 2004 10:46:59 -0000
@@ -0,0 +1,1853 @@
+/*
+ * Copyright (C) the Wine project
+ *
...
+DECL_WINELIB_TYPE_AW(IDirectInputDevice)
+
+/*****************************************************************************
...
+DECL_WINELIB_TYPE_AW(IDirectInputDevice2)
+
...
+DECL_WINELIB_TYPE_AW(IDirectInputDevice7)
+
+#endif /* DI7 */
+
+#if DIRECTINPUT_VERSION >= 0x0800
...
+DECL_WINELIB_TYPE_AW(IDirectInputDevice8)
+
+#endif /* DI8 */
+
...
+DECL_WINELIB_TYPE_AW(IDirectInput)
+
+/*****************************************************************************
...
+DECL_WINELIB_TYPE_AW(IDirectInput2)
+
+#if DIRECTINPUT_VERSION >= 0x0700
+/*****************************************************************************
...
+DECL_WINELIB_TYPE_AW(IDirectInput7)
+
+#endif /* DI7 */
+
+#if DIRECTINPUT_VERSION >= 0x0800
